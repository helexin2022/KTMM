...
global arr
ig=0
objFilePath = 'model3.obj'#читать из файла obj
...
def Area(p1,p2,p3):#Формула Герона
...
#сохранение коэффициентов в csv
A=1
...
init=20,10,10,20,45,20,0,30,50
...
VIEW = np.array([-0.8, 0.8, -0.8, 0.8, 1.0, 20.0])  # 视景体的left/right/bottom/top/near/far六个面 шесть сторон
#VIEW = np.array([-10, 10, -10, 10, 10, 20.0])
SCALE_K = np.array([0.5, 0.5, 0.5])  # 模型缩放比例 Масштабирование модели
EYE = np.array([0.0, 10.0, 20.0])  # 眼睛的位置（默认z轴的正方向）
LOOK_AT = np.array([0.0, 10.0, 0.0])  # 瞄准方向的参考点（默认在坐标原点）Контрольная точка направления прицеливания
EYE_UP = np.array([0.0, 0.0, 10.0])  # 定义对观察者而言的上方（默认y轴的正方向）
WIN_W, WIN_H = 640, 480  # 保存窗口宽度和高度的变量 ширина и высота
...
csv_file=open('Results.csv','r')    #打开csv文件
...
data2=list_to_float #np.array(list_to_float)
#data1=data1[1:,1:]
...
#print((arr))
#print(arr)
Max = max(max(row) for row in data2[:][1:])
Min = min(min(row) for row in data2[:][1:])
print(Max,Min)
#Расстояние, угол места, азимут между глазом и целью наблюдения
def getposture():
    global EYE, LOOK_AT
...
def init():
    ...
    # Используется для включения функции обновления буфера глубины,чтобы реализовать взаимосвязь окклюзии.
    glDepthFunc(GL_LEQUAL)  # 设置深度测试函数 Выражение отношения окклюзии（GL_LEQUAL只是选项之一）
...
def draw():
    ...
    global WIN_W, WIN_H
    global ig
    # 清除屏幕及深度缓存 Очистить экран и кеш глубины
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)

    # 设置投影（透视投影）установить проекцию
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()

    #glFrustum-Вычислить матрицу для реализации перспективной проекции
    #平行投影-投影后的物体大小尺寸不变
    # Параллельная проекция - размер объекта после проекции не меняется
    if WIN_W > WIN_H:
        if IS_PERSPECTIVE:
    ...
    # 设置模型视图Матрица представления модели
    glMatrixMode(GL_MODELVIEW)
    glLoadIdentity()

    # 几何变换 Масштабирование
    ...
    gluLookAt(
        ...
        EYE_UP[0], EYE_UP[1], EYE_UP[2] #Направление, в которое обращена верхняя часть головы
    )
    ...
    glutSwapBuffers()  # 切换缓冲区，以显示绘制内容Переключить буфер, чтобы показать, что рисовать
...
def reshape(width, height):
    ...
    glutPostRedisplay()#перерисовать изображение
...
def keydown(key, x, y):
    ...
    if key in [b'x', b'X', b'y', b'Y', b'z', b'Z']:
        ...
        glutPostRedisplay()
    elif key == b'\r':  # 回车键，视点前进 клавиша Enter, точка зрения вперед
        EYE = LOOK_AT + (EYE - LOOK_AT) * 0.9
        DIST, PHI, THETA = getposture()
        glutPostRedisplay()
    elif key == b'\x08':  # 退格键，视点后退Клавиша Backspace, точка обзора назад;
        EYE = LOOK_AT + (EYE - LOOK_AT) * 1.1
        DIST, PHI, THETA = getposture()
        glutPostRedisplay()
    elif key == b' ':  # 空格键，切换投影模式 Переключить режим проецирования
...
def flutrun():
    ...
    # glutCreateSubWindow(glutCreateWindow('Result'),)
    init()  # 初始化画布
